...
(
    ...
    latest_chat_at timestamp(6) not null,
    created_at timestamp(6) not null,
    updated_at timestamp(6) not null
);

create index ix_chat_threads__userid_latestchatat on chat_threads (user_id, latest_chat_at desc);
create index ix_chat_threads__userid_createdat on chat_threads (user_id, created_at desc);
create index ix_chat_threads__latestchatat on chat_threads (latest_chat_at desc);
